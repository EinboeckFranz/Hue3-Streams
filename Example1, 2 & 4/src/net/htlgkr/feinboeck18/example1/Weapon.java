...
public class Weapon {
    ...
    public Printable printWeaponsNormal = weapons -> {
    ...
    private int[] getBiggestValueOfWeaponParameter(List<Weapon> weapons) {
        return new int[] {
            ...
            weapons.stream().map(Weapon::getDamage).mapToInt(value1 -> value1.toString().length()).max().orElse(0),
            weapons.stream().map(Weapon::getSpeed).mapToInt(value1 -> value1.toString().length()).max().orElse(0),
            weapons.stream().map(Weapon::getStrength).mapToInt(value1 -> value1.toString().length()).max().orElse(0),
            weapons.stream().map(Weapon::getValue).mapToInt(value1 -> value1.toString().length()).max().orElse(0)
        };
    }
    ...
    private void printWeaponsList(List<Weapon> weapons, int[] biggestValuesLength) {
